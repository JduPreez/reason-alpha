(ns ra-datagrid.views.components.datepicker
  ^{:doc
    "Copied from `re-com.datepicker` to allow us to set
     the invalid validation state on the datepicker button
     control, instead of the datepicker calendar table."}
  (:require-macros
...
(defn- anchor-button
  ...
  [shown? model format goog? placeholder width disabled? invalid-feedback]
   ...
   [h-box
    ...
    :children  [[:label {:disabled (deref-or-value disabled?)
                         :class    (str "form-control dropdown-button"
                                        (when (deref-or-value invalid-feedback)
                                          " is-invalid")
                ...
                (if invalid-feedback
                  [:div.invalid-tooltip
                   [:<>
                    (for [f (deref-or-value invalid-feedback)]
...
(def datepicker-dropdown-args-desc
  (when include-args-desc?
    (conj datepicker-args-desc
          ...
          {:name :invalid-feedback :required false                          :type "boolean"})))
       ...
       (or
        ...
        (let [collapse-on-select (fn [new-model]
              ...
              passthrough-args   (-> passthrough-args
                                     (dissoc :format :goog? :no-clip? :placeholder :width :position-offset :position :invalid-feedback)  ;; these keys only valid at this API level
          ...
          [popover-anchor-wrapper
           ...
           :showing? shown?
           :position position
           :anchor   [anchor-button shown? model format goog? placeholder width disabled? invalid-feedback]
